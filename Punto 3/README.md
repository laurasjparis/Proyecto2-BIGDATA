# PUNTO 3 — Ingestión al Datalake (S3)
...
```bash
# 1. Crear bucket S3 con carpetas:
# raw/, trusted/, refined/

# Bucket:
proyecto2-datalake-calidad-aire
```
...
pip3 install boto3 pymysql requests
```
...
# (obtener de AWS Academy → AWS Details)
...
```bash
# 4. Ejecutar script de ingestión
...
```bash
# Si boto3 da NoCredentialsError:
# ejecutar aws configure con credenciales de AWS Academy

# Las credenciales de AWS Academy expiran cada 4 horas
# renovar con aws configure nuevamente

# Si la URL no responde:
